...
运行方式：
    ...
    python3 src/align_evidence.py --project-dir projects/示例企业_2025
"""
...
def build_indicator_collection(
    ...
    Collection 名称："{company_name}_indicators"（如 "示例企业_indicators"）。
...
def run_align_pipeline(paths, rebuild=None, tracker=None):
    ...
    参数：
        ...
        rebuild - 缓存重建级别：None / "embedding" / "chunk" / "extract" / "all"
        tracker - ProgressTracker 实例；None 时自动使用 NullTracker
    """
    ...
    if all_sections is None:
        if force_reextract:
            print(f"  [缓存重建] --rebuild {rebuild}，重新提取")
        ...
        save_vlm_cache(str(paths.vlm_cache))
    ...
    if chunk_records is None:
        if force_rechunk:
            print(f"  [缓存重建] --rebuild {rebuild}，重新分块")
        chunk_records = []
        all_parents = {}
        ...
        for rel_path, sections in all_sections.items():
            ...
            result = make_chunks_from_sections(sections, fr, max_size=_max, min_size=_min)
    ...
    if emb_list is not None:
        ...
        if force_reembed:
            print(f"  [缓存重建] --rebuild {rebuild}，重算 embedding")
...
def main():
    ...
    run_align_pipeline(paths, rebuild=args.rebuild, tracker=tracker)
